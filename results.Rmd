...
Training dataset reads --generated as described in figure \ref{fig:strep}-- are processed by our tool GeDI --see former version \ref{fig:former-gedi}.
CDS coverage are computed and presented in the figure \ref{fig:distrib}.
...
CDS coverage density follow a Gaussian distribution when reads originate from one genome and are aligned on the latter reference genome.
Providing enough reads aligned, no CDS are left unaligned. Hence, they is no difference between blue curves on pane (A) and (B) in \ref{fig:distrib}.
...
This reflects the fraction of genome shared by the reference genome and the genome from which reads where sequenced.
...
For the other parameters of the mixture model, I observed a linear tendency between aligned reads number and the mean $\mu$ or the variance $\sigma^2$ --yet with different coefficient.
