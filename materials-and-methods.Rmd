...
This contribution is noted $\pi_c = \mathbb{P}(Z_{ic} = 1)$. [^4] 
...
In our cases, we choose the following parametric forms for every distribution $f_c$: 3 Gaussian and one log-normal.
...
#####Output files
I decided to rely on RDS files to store all models estimated for the sake of reusability and interoperability.
Reports are automatically generated with a table for every BIC values.
The observed CDS density is plotted and on top the subsequent distributions drawn from the most likely model.
