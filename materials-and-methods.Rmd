---
title: "Materials and Methods"
author: Charlie PAUVERT
classoption: a4paper
fontsize: 12pt
lang: en
output:
  pdf_document:
    pandoc_args: [ "--latex-engine=/usr/bin/pdflatex" ] 
    toc: yes
    number_section: true
header-includes:
 - \usepackage{libertine}
--- 



# Materials and methods (7p)

* How do I work?
* What tools and data did I used?
* **Insist on EM algorithm** according to NV remarks?
* New python librairies used in GeDI.
* Private GitHub for report.

```{r gedi,  engine="tikz",engine.opts=list(template="template-tikz2pdf.tex"),echo=FALSE, fig.align="center",fig.scap="Principles of metagenomics analysis tool: GeDI.", fig.cap="Principles of metagenomics analysis tool: GeDI.",cache=TRUE }
\input{figures/fig-gedi}
```
