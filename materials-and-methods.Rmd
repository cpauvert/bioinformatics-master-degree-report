...
```{r coverage,  engine="tikz",engine.opts=list(template="template-tikz2pdf.tex"),echo=FALSE, fig.align="center",fig.scap="Genome coverage after reads alignement.", fig.cap="Genome coverage after reads alignement. In our metagenomics analysis tool -- GeDI -- we discrimate (1) total coverage yield by the reads number for each genome base and (2) mismatches coverage concerning the same metric only for reads aligned with one to three mismatches compared to the reference.",cache=FALSE }
