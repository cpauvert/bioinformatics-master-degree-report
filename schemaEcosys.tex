...
\matrix [matrix of nodes, nodes in empty cells,nodes={align=center},row sep = 2em,
		...
		column 2/.style ={text width = 17em},
		column sep={3em}] (m) {%
			% row 1
|[text width =2cm, text centered]|  {Microbial ecosystem} \&   |[rectangle, draw=Igreen, fill=Igreen!25,align=left]| {$\bullet$ Which micro-organisms are present? \\ $\bullet$ What functions are associated?} \& \\
			% row 2
 \&  |[rectangle, draw=Igreen, label={\textbf{Metagenomics}}]| {Capture and analysis of \\ecosystem genomics information}\& \\
			% row 3
...
\draw[thick , ->, >=stealth] (m-1-1) -- (m-1-2);
\draw[thick , ->, >=stealth] (m-1-1) |- (m-2-2);
\draw[thick , ->, >=stealth] (fitP) -- node[above,midway] {DNA extraction} (fitP-|m-3-1.east);
\draw[thick , ->, >=stealth] (fitP) -- node[above,midway] {DNA extraction} (fitP-|m-3-3.west);
%
...
\node[draw=lightgray, inner sep=3pt,fit={(id) (id16)},label={above:Post-sequencing \textbf{methods}}]  {};
\node[draw=lightgray, inner sep=10pt,fit={(cmp) (alg) (alg16)},label={below left:Micro-organisms identification \textbf{strategies} examples}] {};
\end{tikzpicture}
